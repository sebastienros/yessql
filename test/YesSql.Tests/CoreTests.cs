    ...
    public abstract class CoreTests : IDisposable
    {
        ...
        public async Task ShouldIndexWithDateTime()
        {
            ...
            using (var session = _store.CreateSession())
            {
                ...
                Assert.Equal(4, await tx.Connection.ExecuteScalarAsync<int>("select count(*) from \"ArticleByPublishedDate\" where (\"ArticleByPublishedDate\".\"PublishedDateTime\" = '2011-11-01T00:00:00Z')", tx));

                Assert.Equal(6, (await session.QueryIndexAsync<ArticleByPublishedDate>(x => x.PublishedDateTime == new DateTime(2011, 11, 1, 0, 0, 0, DateTimeKind.Utc)).List()).Count());
                Assert.Equal(5, await session.QueryIndexAsync<ArticleByPublishedDate>(x => x.PublishedDateTime == new DateTime(2011, 11, 1, 0, 0, 0, DateTimeKind.Utc)).Count());
            }
        }
